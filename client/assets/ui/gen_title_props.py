...
def candle_rack(w=520, h=300):
    ...
    # Two ranks of tapers at wildly differing heights — the irregularity IS the prop.
    for rank, (yoff, scale, count) in enumerate(((0.0, 1.0, 13), (-h * 0.05, 0.86, 12))):
        for i in range(count):
            fx = (i + 0.5) / count
            px_ = w * (0.11 + 0.78 * fx) + (rank * w * 0.028)
            tall = h * (0.16 + 0.30 * _h(i, rank, 11)) * scale
            cw = w * 0.0105 * scale
    ...
    def run_shade(x, y, d):
        return A.lerp_rgb(WAX_MID, WAX_DK, 0.35 + 0.30 * _h(int(x) // 4, 0, 13))
    ...
    lip = tray_y + h * 0.028
    for i in range(11):
        rx_ = w * (0.13 + 0.74 * _h(i, 1, 17))
        drop = h * (0.018 + 0.055 * _h(i, 2, 19))
        rw = w * (0.006 + 0.006 * _h(i, 3, 23))
...
def brazier(w=300, h=280):
    c = Canvas(w, h)
    cx = w * 0.5
    rim_y, rim_rx, rim_ry = h * 0.34, w * 0.40, h * 0.10
    leg = w * 0.030

    for th in (math.tau * 0.08, math.tau * 0.40, math.tau * 0.72):
    ...
    def coal_shade(x, y, d):
        ...
        n = _h(int(x) // 5, int(y) // 4, 7)
        ash = _h(int(x) // 9, int(y) // 7, 11)
...
def main():
    ...
    cen = censer()
    A.write_png("title/censer.png", cen.w, cen.h, cen.px)
    cha = chandelier()
    A.write_png("title/chandelier.png", cha.w, cha.h, cha.px)
    rack = candle_rack()
    A.write_png("title/candle_rack.png", rack.w, rack.h, rack.px)
    bra = brazier()
    A.write_png("title/brazier.png", bra.w, bra.h, bra.px)
    print("gen_title_props OK — censer, chandelier, candle rack, brazier (unlit, alpha).")

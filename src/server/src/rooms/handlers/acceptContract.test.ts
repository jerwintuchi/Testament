...
describe('handleAcceptContract', () => {
  ...
  it('emits LOBBY_ERROR NOT_LEADER for a non-leader socket', () => {
    ...
    const room = mgr.getRoomBySocketId('host')!;
    room.players.push({ playerId: 'p2', displayName: 'P2', socketId: 'p2-sock', isLeader: false, readyState: true, disconnectedAt: null, rank: 'HIEROPHANT', perceivedChannels: [], bag: [], pos: null, moveIntent: { dx: 0, dy: 0 } });
  ...
  it('succeeds with a not-ready disconnected player in the room', () => {
    ...
    room.players.push({
      ...
      readyState: false, disconnectedAt: Date.now(), rank: 'HIEROPHANT', perceivedChannels: [], bag: [], pos: null, moveIntent: { dx: 0, dy: 0 },
    });
    const { fn: broadcast, calls } = makeBroadcast();
    handleAcceptContract('host', mgr, () => {}, broadcast);
